...
Specifically, an RP **MUST** only rely on `amr_details` when the Claim is:
...
When Authentication Method requirements are expressed by the RP, the OP may authenticate the End-User using the best available Authentication Method according to its capabilities and policies unless the top-level `amr_details` request is essential. For an essential request, the OP **MUST** enforce the Claim and expression requirements.

Regardless of OP behavior, RPs **MUST** validate and evaluate the returned `amr_details` Claim against their local authentication policies, particularly when the request was non-essential or the OP did not advertise request-processing support.

If the Authentication Methods used do not satisfy the RP's local requirements, the RP **MUST NOT** grant access solely based on successful completion of the OpenID Connect flow.
...
OPs **SHOULD** apply data minimization principles when returning `amr_details` by limiting optional Authentication Method Metadata and Authentication Method Properties to what is necessary.
...
## Disclosure Policy and Essential Requests {#sec-disclosure-policy}

For the purposes of this specification, an applicable disclosure policy is a policy that constrains the information that an OP may disclose for a particular authorization transaction. Such a policy may derive from the OP's internal policy, a trust framework, a contractual agreement, or a regulatory obligation.

An applicable disclosure policy **MUST** be evaluated before an essential expression is considered satisfied. If the policy prevents the OP from returning the complete Authentication Event or a locally essential field, the essential request is not satisfied.

## Replay and Temporal Considerations

The `amr_details` Claim includes temporal information describing when Authentication Methods were performed or validated. RPs **MAY** use such information to evaluate the freshness of the Authentication Context according to their security policies.
